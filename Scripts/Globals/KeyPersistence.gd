...
var default_keymaps: Dictionary
...
func _ready() -> void:
	...
	default_keymaps = keymaps.duplicate(true)
	
	load_keymap()


func get_default_action(action: StringName) -> InputEvent:
	if default_keymaps.has(action):
		return default_keymaps[action]
	else:
		return null
