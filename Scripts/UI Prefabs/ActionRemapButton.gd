...
func _unhandled_key_input(input_event: InputEvent) -> void:
	...
	if input_event is InputEventKey and input_event.keycode != KEY_ENTER and input_event.keycode != KEY_F8:
...
func display_current_key() -> void:
	...
	text = current_key.replace(" - Physical", "")


func reset() -> void:
	remap_action_to(KeyPersistence.get_default_action(action))
	display_current_key()
