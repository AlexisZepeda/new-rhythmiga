...
func evaluate_release(param_delta: float, key: Key, note_manager_key: Key) -> bool:
	if key != note_manager_key:
		print("Wrong param key %s note manager key %s" % [OS.get_keycode_string(key), OS.get_keycode_string(note_manager_key)])
