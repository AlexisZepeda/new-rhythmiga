...
@export var bpm: float = 65.0:
	set(value):
		bpm = value
		EmbeddedGlobalSettings.sixteenth_duration = 60 / bpm / Enums.Duration.SIXTEENTH
...
@export var note_duration: Enums.Duration = Enums.Duration.QUARTER
...
func load_stream(new_stream: AudioStream) -> void:
	stream = new_stream
	loaded_new_stream.emit()


func play_conductor(time: float) -> void:
	var filter_args := {
...
func stop_conductor() -> void:
	...
	_song_time_begin = 0
	_song_time_system = -100
	_filtered_audio_system_delta = 0
	_is_playing = false
...
func get_beat(time: float) -> float:
	var song_time: float = time + _filtered_audio_system_delta
...
## Returns the duration of one quarter note (in seconds).
